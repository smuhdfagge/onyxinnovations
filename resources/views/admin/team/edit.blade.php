...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-6">Edit Team Member</h2>
        ...
        <form action="{{ route('admin.team.update', $team) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                @if($team->photo)
                <div class="mb-3">
                    <img src="{{ Storage::url($team->photo) }}" alt="{{ $team->name }}" class="h-24 w-24 rounded-full object-cover">
                </div>
                @endif
                <input type="file" name="photo" id="photo" accept="image/*"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                <p class="mt-1 text-sm text-gray-500">Leave empty to keep current photo. Recommended: Square image, at least 400x400px</p>
                @error('photo')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="bio" id="bio" rows="4"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Brief biography...">{{ old('bio', $team->bio) }}</textarea>
                @error('bio')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center space-x-6">
                <label class="flex items-center">
                    ...
                    <input type="checkbox" name="is_leadership" value="1" {{ old('is_leadership', $team->is_leadership) ? 'checked' : '' }}
