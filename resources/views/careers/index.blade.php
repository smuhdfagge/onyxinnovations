...
@section('title', 'Careers - Onyx Innovations Ltd')
@section('meta_description', 'Join our team at Onyx Innovations. Explore exciting career opportunities in software development, cloud computing, AI, and more.')
...
<section class="py-24 bg-gray-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-12 flex flex-wrap gap-4 justify-center">
            <form method="GET" class="flex flex-wrap gap-4 items-center">
                ...
                <select name="location" onchange="this.form.submit()" class="px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 bg-white">
                    ...
                    <option value="onsite" {{ request('location') == 'onsite' ? 'selected' : '' }}>On-site</option>
                    <option value="hybrid" {{ request('location') == 'hybrid' ? 'selected' : '' }}>Hybrid</option>
                </select>
                @if(request()->hasAny(['category', 'type', 'location']))
                <a href="{{ route('careers.index') }}" class="text-blue-600 hover:text-blue-700 font-medium">Clear Filters</a>
                @endif
            </form>
        </div>
        ...
        <div class="space-y-6">
            @forelse($jobs ?? [] as $job)
            <a href="{{ route('careers.show', $job) }}" class="block bg-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all border border-gray-100 group">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div class="flex items-center space-x-4">
                        @if($job->salary_range)
                        <span class="text-gray-500">{{ $job->salary_range }}</span>
                        @endif
            ...
            <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <a href="{{ route('contact') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 text-white rounded-full font-semibold hover:bg-blue-700 transition-colors">
                        Apply Now
                    </a>
                </div>
            </div>
...
<section class="py-24 bg-blue-600">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <a href="{{ route('contact') }}?type=careers" class="inline-flex items-center justify-center px-8 py-4 bg-white text-blue-600 rounded-full font-semibold hover:bg-blue-50 transition-colors shadow-lg">
            Submit Your Resume
